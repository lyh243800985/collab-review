  ...
  if (msg.cmd === 'tabs') {
    try {
      if (msg.method === 'switch') {
        ...
        return { ok: true };
      } else if (msg.method === 'updateGroup') {
        const group = await chrome.tabGroups.update(msg.groupId, {
          title: msg.title,
          color: msg.color,
          collapsed: msg.collapsed
        });
        return { ok: true, data: { id: group.id, title: group.title, color: group.color, collapsed: group.collapsed, windowId: group.windowId } };
      } else if (msg.method === 'createGroup') {
        const urls = Array.isArray(msg.urls) ? msg.urls.filter(Boolean) : [];
        if (!urls.length) return { ok: false, error: 'tabs.createGroup requires at least one URL' };

        const windowId = msg.windowId || (await chrome.windows.getLastFocused()).id;
        const createdTabs = [];
        try {
          // Create every test page inactive so audit setup never steals focus
          // from the user's current development tab.
          for (const url of urls) {
            createdTabs.push(await chrome.tabs.create({ windowId, url, active: false }));
          }
          const tabIds = createdTabs.map(tab => tab.id);
          const groupId = await chrome.tabs.group({ tabIds, createProperties: { windowId } });
          const group = await chrome.tabGroups.update(groupId, {
            title: msg.title || 'Agent Tasks',
          ...
          return {
            ok: true,
            data: {
              group: { id: group.id, title: group.title, color: group.color, collapsed: group.collapsed, windowId: group.windowId },
              tabs: createdTabs.map(tab => ({ id: tab.id, url: tab.pendingUrl || tab.url || '', title: tab.title || '' }))
            }
          };
        } catch (error) {
          // Group creation is atomic from the caller's perspective: remove any
          // partially created tabs before returning the failure.
        ...
        // Parallel auditors need separate pages; creating inactive tabs avoids
        // stealing focus from the user's current development environment.
  ...
  try {
    for (const c of msg.commands) {
      ...
      if (c.cmd === 'cookies') {
        ...
        // Keep batch tab commands consistent with their single-command form.
        // Parallel audit flows use updateGroup/createGroup for lifecycle marking;
        // silently degrading those commands to a tab listing loses the action.
...
const HEALTH_URL = 'http://127.0.0.1:18766/link';
...
async function isServerAlive() {
  ...
  const timeoutId = setTimeout(() => ctrl.abort(), 800);
  try {
    // Probe the bridge's real HTTP endpoint before constructing WebSocket.
    // A caught fetch failure stays out of Chrome's extension error list, while
    // a failed WebSocket handshake is always recorded as ERR_CONNECTION_REFUSED.
...
// Initial connect + wake-up hooks. Always probe first so starting Chrome before
// Codex does not leave a permanent ERR_CONNECTION_REFUSED entry in the UI.
connectWhenReady();
chrome.runtime.onStartup.addListener(() => connectWhenReady());
chrome.runtime.onInstalled.addListener(() => connectWhenReady());
